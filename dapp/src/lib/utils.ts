...
export function timeLeft(expiry: number): string {
  ...
  if (diff < 86_400)
    return `${Math.floor(diff / 3_600)}h ${Math.floor((diff % 3_600) / 60)}m`;
...
export const X_HASHTAGS = "#GoodDrops #GoodDollar";
...
export function buildPrivateChainHint(
  hint: string,
  nextDropId: string,
): string {
